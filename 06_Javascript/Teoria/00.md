...
```javascript
// Crear una cookie
document.cookie = "nombreUsuario=Juan; expires=Fri, 01 Jan 2025 12:00:00 UTC; path=/";
...
1. **Crear una cookie en JavaScript**:
   ```javascript
   document.cookie = "usuario=Juan; expires=" + new Date(Date.now() + 86400000).toUTCString() + "; path=/";
   ```
